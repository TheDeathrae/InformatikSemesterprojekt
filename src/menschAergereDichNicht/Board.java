...
public class Board {
	...
	void bewegung(int start, int ende) {																						// f�hrt Bewegung von Steinen aus
		...
		if(endObjekt == 0) {
			...
			if(endObjekt == 1) {
				for(int i = 56; i <= 59; i++) {
					int a = 0;																					// sorgt daf�r, dass nicht mehrere Pl�tze besetzt werden
					if(a == 0) {
						if(positionen[i] == 0) {
